...
class test_resampler(TestCase):
    def test_systematic_resampler(self):
        ...
        for i in range(len(samples)):
            # Fix: Use int(samples[i][0]) instead of samples[i] directly
            sample_value = int(samples[i][0])
            occs = [c[sample_value] if sample_value in c else 0 for c in sample_counts]
        ...
        for n, i in enumerate(samples):
            # Fix: Use int(i[0]) instead of i directly
            sampled = all_counts[int(i[0])]
